...
namespace Arcomage.Scripts
{
    public class Settings : Control
    {
        ...
        public override void _Ready()
        {
            ...
            {
                case Config.Locale.en:
                    TranslationServer.SetLocale("en"); 
                    break;
                case Config.Locale.ru:
                    TranslationServer.SetLocale("ru"); 
                    break;
                case Config.Locale.uk:
                    TranslationServer.SetLocale("uk");
                    break;
                case Config.Locale.pl:
                    TranslationServer.SetLocale("pl");
                    break;
                case Config.Locale.da:
        ...
        private void SaveSettings()
        {
            ...
            {
                // Window settings
                c.SetValue("WINDOW", "fullscreen", Config.Fullscreen);
                c.SetValue("WINDOW", "borderless", Config.Borderless);
                c.SetValue("WINDOW", "width", Config.WindowWidth);
                c.SetValue("WINDOW", "height", Config.WindowHeight);
                c.SetValue("WINDOW", "vsync", Config.Vsync);
                c.SetValue("WINDOW", "intro_skip", Config.IntroSkip);
                // Audio settings
                c.SetValue("AUDIO", "master", Config.MasterVolume);
                c.SetValue("AUDIO", "music", Config.MusicVolume);
                c.SetValue("AUDIO", "sound", Config.SoundVolume);
                c.SetValue("AUDIO", "mute", Config.MuteSound);
                // Starting conditions
                c.SetValue("START", "singleplayer", Config.Singleplayer);
                c.SetValue("START", "single_click_mode", Config.SingleClick);
                c.SetValue("START", "tower_levels", Config.TowerLevels);
                c.SetValue("START", "wall_levels", Config.WallLevels);
                c.SetValue("START", "quarry_levels", Config.QuarryLevels);
                c.SetValue("START", "brick_quantity", Config.BrickQuantity);
                c.SetValue("START", "magic_levels", Config.MagicLevels);
                c.SetValue("START", "gem_quantity", Config.GemQuantity);
                c.SetValue("START", "dungeon_levels", Config.DungeonLevels);
                c.SetValue("START", "recruit_quantity", Config.RecruitQuantity);
                // Play conditions
                c.SetValue("PLAY", "auto_bricks", Config.AutoBricks);
                c.SetValue("PLAY", "auto_gems", Config.AutoGems);
                c.SetValue("PLAY", "auto_recruits", Config.AutoRecruits);
                c.SetValue("PLAY", "cards_in_hand", Config.CardsInHand);
                c.SetValue("PLAY", "ai", Config.CurrentAiType);
                // Victory conditions
                c.SetValue("VICTORY", "tower_victory", Config.TowerVictory);
                c.SetValue("VICTORY", "resource_victory", Config.ResourceVictory);
                // Tavern presets
                c.SetValue("TAVERN", "preset", Config.CurrentTavern);
                // Language settings
                c.SetValue("LANGUAGE", "locale", Config.CurrentLocale);
                // Player settings
                c.SetValue("PLAYER", "nickname", Config.Nickname);
                c.Save(_configPath);
            }
            else
            {
        ...
        private void LoadSettings()
        {
            ...
            Config.Fullscreen = (bool)c.GetValue("WINDOW", "fullscreen");
            _fullscreenButton.Pressed = Config.Fullscreen;
            HasKeyElseSet(c,"WINDOW", "borderless", _borderlessButton.Pressed);
            Config.Borderless = (bool)c.GetValue("WINDOW", "borderless");
            _borderlessButton.Pressed = Config.Borderless;
            HasKeyElseSet(c,"WINDOW", "width", int.Parse(_windowWidthEdit.Text));
            Config.WindowWidth = (int)c.GetValue("WINDOW", "width");
            _windowWidthEdit.Text = Config.WindowWidth.ToString();
            HasKeyElseSet(c,"WINDOW", "height", int.Parse(_windowHeightEdit.Text));
            Config.WindowHeight = (int)c.GetValue("WINDOW", "height");
            _windowHeightEdit.Text = Config.WindowHeight.ToString();
            HasKeyElseSet(c,"WINDOW", "vsync", _vsyncButton.Pressed);
            Config.Vsync = (bool)c.GetValue("WINDOW", "vsync");
            _vsyncButton.Pressed = Config.Vsync;
            HasKeyElseSet(c,"WINDOW", "intro_skip", _introSkipButton.Pressed);
            Config.IntroSkip = (bool)c.GetValue("WINDOW", "intro_skip");
            _introSkipButton.Pressed = Config.IntroSkip;
            // Audio settings
            HasKeyElseSet(c,"AUDIO", "master", _masterVolume.Value);
            Config.MasterVolume = Convert.ToDouble(c.GetValue("AUDIO", "master"));
            _masterVolume.Value = Config.MasterVolume;
            HasKeyElseSet(c,"AUDIO", "music", _musicVolume.Value);
            Config.MusicVolume = Convert.ToDouble(c.GetValue("AUDIO", "music"));
            _musicVolume.Value = Config.MusicVolume;
            HasKeyElseSet(c,"AUDIO", "sound", _soundVolume.Value);
            Config.SoundVolume = Convert.ToDouble(c.GetValue("AUDIO", "sound"));
            _soundVolume.Value = Config.SoundVolume;
            HasKeyElseSet(c,"AUDIO", "mute", _muteSound.Pressed);
            Config.MuteSound = (bool)c.GetValue("AUDIO", "mute");
            _muteSound.Pressed = Config.MuteSound;
            // Starting conditions
            HasKeyElseSet(c,"START", "singleplayer", _singleplayerButton.Pressed);
            Config.Singleplayer = (bool)c.GetValue("START", "singleplayer");
            _singleplayerButton.Pressed = Config.Singleplayer;
            HasKeyElseSet(c,"START", "single_click_mode", _singleClickButton.Pressed);
            Config.SingleClick = (bool)c.GetValue("START", "single_click_mode");
            _singleClickButton.Pressed = Config.SingleClick;
            HasKeyElseSet(c,"START", "tower_levels", int.Parse(_towerLevels.Value.ToString(_invariantCulture)));
            Config.TowerLevels = (int)c.GetValue("START", "tower_levels");
            _towerLevels.Value = Config.TowerLevels;
            HasKeyElseSet(c,"START", "wall_levels", int.Parse(_wallLevels.Value.ToString(_invariantCulture)));
            Config.WallLevels = (int)c.GetValue("START", "wall_levels");
            _wallLevels.Value = Config.WallLevels;
            HasKeyElseSet(c,"START", "quarry_levels", int.Parse(_quarryLevels.Value.ToString(_invariantCulture)));
            Config.QuarryLevels = (int)c.GetValue("START", "quarry_levels");
            _quarryLevels.Value = Config.QuarryLevels;
            HasKeyElseSet(c,"START", "brick_quantity", int.Parse(_brickQuantity.Value.ToString(_invariantCulture)));
            Config.BrickQuantity = (int)c.GetValue("START", "brick_quantity");
            _brickQuantity.Value = Config.BrickQuantity;
            HasKeyElseSet(c,"START", "magic_levels", int.Parse(_magicLevels.Value.ToString(_invariantCulture)));
            Config.MagicLevels = (int)c.GetValue("START", "magic_levels");
            _magicLevels.Value = Config.MagicLevels;
            HasKeyElseSet(c,"START", "gem_quantity", int.Parse(_gemQuantity.Value.ToString(_invariantCulture)));
            Config.GemQuantity = (int)c.GetValue("START", "gem_quantity");
            _gemQuantity.Value = Config.GemQuantity;
            HasKeyElseSet(c,"START", "dungeon_levels", int.Parse(_dungeonLevels.Value.ToString(_invariantCulture)));
            Config.DungeonLevels = (int)c.GetValue("START", "dungeon_levels");
            _dungeonLevels.Value = Config.DungeonLevels;
            HasKeyElseSet(c,"START", "recruit_quantity", int.Parse(_recruitQuantity.Value.ToString(_invariantCulture)));
            Config.RecruitQuantity = (int)c.GetValue("START", "recruit_quantity");
            _recruitQuantity.Value = Config.RecruitQuantity;
            // Play conditions
            HasKeyElseSet(c,"PLAY", "auto_bricks", int.Parse(_autoBricks.Value.ToString(_invariantCulture)));
            Config.AutoBricks = (int)c.GetValue("PLAY", "auto_bricks");
            _autoBricks.Value = Config.AutoBricks;
            HasKeyElseSet(c,"PLAY", "auto_gems", int.Parse(_autoGems.Value.ToString(_invariantCulture)));
            Config.AutoGems = (int)c.GetValue("PLAY", "auto_gems");
            _autoGems.Value = Config.AutoGems;
            HasKeyElseSet(c,"PLAY", "auto_recruits", int.Parse(_autoRecruits.Value.ToString(_invariantCulture)));
            Config.AutoRecruits = (int)c.GetValue("PLAY", "auto_recruits");
            _autoRecruits.Value = Config.AutoRecruits;
            HasKeyElseSet(c,"PLAY", "cards_in_hand", int.Parse(_cardsInHand.Value.ToString(_invariantCulture)));
            Config.CardsInHand = (int)c.GetValue("PLAY", "cards_in_hand");
            _cardsInHand.Value = Config.CardsInHand;
            HasKeyElseSet(c,"PLAY", "ai", _ai.Selected);
            Config.CurrentAiType = (Config.AiType)c.GetValue("PLAY", "ai");
            _ai.Selected = (int)Config.CurrentAiType;
            // Victory conditions
            HasKeyElseSet(c,"VICTORY", "tower_victory", int.Parse(_winTower.Value.ToString(_invariantCulture)));
            Config.TowerVictory = (int)c.GetValue("VICTORY", "tower_victory");
            _winTower.Value = Config.TowerVictory;
            HasKeyElseSet(c,"VICTORY", "resource_victory", int.Parse(_winResources.Value.ToString(_invariantCulture)));
            Config.ResourceVictory = (int)c.GetValue("VICTORY", "resource_victory");
            _winResources.Value = Config.ResourceVictory;
            // Tavern presets
            HasKeyElseSet(c,"TAVERN", "preset", _tavernPreset.Selected);
            Config.CurrentTavern = (Config.Tavern)c.GetValue("TAVERN", "preset");
            _tavernPreset.Selected = (int)Config.CurrentTavern;
            // Language settings
            HasKeyElseSet(c,"LANGUAGE", "locale", _language.Selected);
            Config.CurrentLocale = (Config.Locale)c.GetValue("LANGUAGE", "locale");
            _language.Selected = (int)Config.CurrentLocale;
            // Player settings
            HasKeyElseSet(c,"PLAYER", "nickname", _nickname.Text);
            Config.Nickname = (string)c.GetValue("PLAYER", "nickname");
        ...
        private void OnIntroSkipButtonToggled(bool toggle) => Config.IntroSkip = toggle;
        ...
        private void OnTowerLevelsValueChanged(float value) => Config.TowerLevels = (int)value;
        private void OnWallLevelsValueChanged(float value) => Config.WallLevels = (int)value;
        private void OnQuarryLevelsValueChanged(float value) => Config.QuarryLevels = (int)value;
        private void OnMagicLevelsValueChanged(float value) => Config.MagicLevels = (int)value;
        private void OnDungeonLevelsValueChanged(float value) => Config.DungeonLevels = (int)value;
        private void OnBrickQuantityValueChanged(float value) => Config.BrickQuantity = (int)value;
        private void OnGemQuantityValueChanged(float value) => Config.GemQuantity = (int)value;
        private void OnRecruitQuantityValueChanged(float value) => Config.RecruitQuantity = (int)value;
        
        private void OnAutoGetBricksValueChanged(float value) => Config.AutoBricks = (int)value;
        private void OnAutoGetGemsValueChanged(float value) => Config.AutoGems = (int)value;
        private void OnAutoGetRecruitsValueChanged(float value) => Config.AutoRecruits = (int)value;
        private void OnCardsInHandValueChanged(float value) => Config.CardsInHand = (int)value;
        private void OnAiModeChanged(int index) => Config.CurrentAiType = (Config.AiType)index;

        private void OnTowerVictoryValueChanged(float value) => Config.TowerVictory = (int)value;
        private void OnResourceVictoryValueChanged(float value) => Config.ResourceVictory = (int)value;

        private void OnTavernPresetSelected(int index) => Config.CurrentTavern = (Config.Tavern)index;

        private void OnLanguageSelected(int index)
        {
            switch ((Config.Locale)index)
            {
                case Config.Locale.en:
                    TranslationServer.SetLocale("en");
                    break;
        ...
        private void OnNicknameTextChanged(string newText) => Config.Nickname = newText;
    }
}
